...
void disassociate_bucket(Connection *c) {
    LOG_WARNING(NULL,"disassociate_bucket conn = %p",(void*)c);
    Bucket &b = all_buckets.at(c->getBucketIndex());
    cb_mutex_enter(&b.mutex);
    LOG_WARNING(NULL,"disassociate_bucket conn = %p clients= %d ",(void*)c, b.clients);
    b.clients--;
...
bool associate_bucket(Connection *c, const char *name) {
    LOG_WARNING(NULL,"associate_bucket conn = %p name = %s",(void*)c, name);
    bool found = false;
    ...
    for (int ii = 1; ii < settings.max_buckets && !found; ++ii) {
        ...
        if (b.state == BucketState::Ready && strcmp(b.name, name) == 0) {
            LOG_WARNING(NULL,"associate_bucket (for loop) conn = %p clients= %d ",(void*)c, b.clients);
            b.clients++;
            c->setBucketIndex(ii);
    ...
    if (!found) {
        ...
        cb_mutex_enter(&b.mutex);
        LOG_WARNING(NULL,"associate_bucket (if !found) conn = %p clients= %d ",(void*)c, b.clients);
        b.clients++;
        cb_mutex_exit(&b.mutex);
        c->setBucketIndex(0);
        c->setBucketEngine(b.engine);
...
void associate_initial_bucket(Connection *c) {
    LOG_WARNING(NULL,"associate_initial_bucket conn = %p",(void*)c);
    Bucket &b = all_buckets.at(0);
    cb_mutex_enter(&b.mutex);
    LOG_WARNING(NULL,"associate_initial_bucket conn = %p clients= %d ",(void*)c, b.clients);
    b.clients++;
    cb_mutex_exit(&b.mutex);
...
void DestroyBucketThread::destroy() {
    LOG_WARNING(connection,"DestroyBucketThread::destroy conn = %p", connection);
    ...
    if (connection != nullptr && idx == connection->getBucketIndex()) {
        LOG_WARNING(connection,"DestroyBucketThread::destroy calling disassociate_bucket conn = %p", connection);
        disassociate_bucket(connection);
    } else
    {
        LOG_WARNING(connection,"DestroyBucketThread::destroy NOT calling disassociate_bucket conn = %p", connection);
    }
